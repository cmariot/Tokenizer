...
contract AlphaCoin42 {
    ...
    */
    function increaseAllowance(address spender_, uint256 addedValue) public returns (bool) {
        _allowances[msg.sender][spender_] += addedValue;
        emit Approval(msg.sender, spender_, _allowances[msg.sender][spender_]);
        return true;
    }

    function decreaseAllowance(address spender_, uint256 subtractedValue) public returns (bool) {
        require(_allowances[msg.sender][spender_] >= subtractedValue, "ERC20: decreased allowance below zero");
        _allowances[msg.sender][spender_] -= subtractedValue;
        emit Approval(msg.sender, spender_, _allowances[msg.sender][spender_]);
        return true;
    }
